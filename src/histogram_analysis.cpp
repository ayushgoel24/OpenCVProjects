...
void imageNormalization(char **argv)
{
    ...
    cv::normalize(src_img, normalized_src_img, lower_range_boundary, upper_range_boundary, cv::NORM_MINMAX, CV_64FC3);
...
void histogramCalculation(char **argv)
{
    ...
    Mat b_hist, g_hist, r_hist, b_hist_normalized, g_hist_normalized, r_hist_normalized;
    ...
    normalize(b_hist, b_hist_normalized, 0, histImage.rows, NORM_MINMAX, -1, Mat() );
    normalize(g_hist, g_hist_normalized, 0, histImage.rows, NORM_MINMAX, -1, Mat() );
    normalize(r_hist, r_hist_normalized, 0, histImage.rows, NORM_MINMAX, -1, Mat() );
    for( int i = 1; i <     bins; i++ )
    {

        line( histImageNormalized, Point( bin_w*(i-1), hist_h - cvRound(b_hist_normalized.at<float>(i-1)) ),
              Point( bin_w*(i), hist_h - cvRound(b_hist_normalized.at<float>(i)) ),
              Scalar( 255, 0, 0), 2, 8, 0  );
        line( histImageNormalized, Point( bin_w*(i-1), hist_h - cvRound(g_hist_normalized.at<float>(i-1)) ),
              Point( bin_w*(i), hist_h - cvRound(g_hist_normalized.at<float>(i)) ),
              Scalar( 0, 255, 0), 2, 8, 0  );
        line( histImageNormalized, Point( bin_w*(i-1), hist_h - cvRound(r_hist_normalized.at<float>(i-1)) ),
