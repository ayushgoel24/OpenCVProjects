...
#include <opencv/highgui.h> //GUI, Image and Video I/O
...
/*  OpenCV Conventions:
    cvsomething: a function
    Cvsomething: class or struct
    CV_something enum or macro

    examples:

    CV_32F is enum is enum equal to 5
    CvMat struct or class
    CvPoint struct
    cvPoint() function
    cvCreateMat is function
*/
...
void CStyleDisplayImage(IplImage  *img,std::string Title)
{
	//Create the window for showing the image,
	IplImage * img_clone=cvCloneImage(img);

	cvNamedWindow( Title.c_str(), CV_WINDOW_AUTOSIZE );
	cvShowImage( Title.c_str(), img_clone );
	cvWaitKey(0);

//	Release the created elements which have been used (e.g. windows, image structures etc).
	cvReleaseImage( &img_clone );
	cvDestroyWindow( Title.c_str() );
}

void CStyleCreateImage()
{
/*
...
|-- int depth;			// Channel depth in bits + the optional sign bit ( IPL_DEPTH_SIGN ). The supported depths are:: 
...
|				// color images are stored in BGR order
...
void CStyleCloneImage(IplImage* imgsrc,IplImage* imgclone)
{
    imgclone=cvCloneImage(imgsrc);
}

void CStyleLoadImage(std::string ImagePath,int  flag, IplImage * &img)
{
//	By default, images are loaded as three-channel images with 	8 bits per channel
//	color images are stored in BGR order!

/*
	CV_LOAD_IMAGE_ANYDEPTH:
		can be added to allow loading of non-8-bit images.

	CV_LOAD_IMAGE_COLOR:
		This means that, regardless of the number of channels in the image file, the image will be converted to three channels if needed.

	CV_LOAD_IMAGE_GRAYSCALE:
		automatically converts any image into a single-channel image

	CV_LOAD_IMAGE_ANYCOLOR:
		will simply load the image as it is stored in the file.

	CV_LOAD_IMAGE_UNCHANGED:
		If you want both the color and depth to be loaded exactly "as is", you could instead use the all-purpose flag
		CV_LOAD_IMAGE_UNCHANGED.
*/

	img=cvLoadImage(ImagePath.c_str() ,flag) ;
	cout<< img<<endl;
	cout<<"Number of color channels (1,2,3 or 4) =" << img->nChannels <<endl;
	cout<<"Number of bits for each pixel: " << img->depth <<endl;
	cout<<"Height: " << img->height <<endl;
	cout<<"Width: " << img->width <<endl;
	cout<<"dataOrder:\n 0 means interleaved color channels,\n 1 means separate color channels \n color images are stored in BGR order" <<endl;
	cout<<"dataOrder= " <<img->dataOrder <<endl;
	//because images are interleaved, so the colors for each pixel is located one after each other in B,G,R sequence
	cout<<"img->widthStep=img->width*img->nChannels " <<img->widthStep <<endl;
}

void CStyleLoadImage_Test(char** argv)
{
	int flag=CV_LOAD_IMAGE_UNCHANGED ;
	IplImage *img;
	std::string ImagePath=argv[1];
	CStyleLoadImage(ImagePath, flag, img);
	std::string Title=ImagePath;
	CStyleDisplayImage(img,Title);

//	Release the created elements which have been used (e.g. windows, image structures etc).
	cvReleaseImage( &img );
}

void CStyleSaveImage()
{
/*
    Writing an image to a file:
    The output file format is determined based on the file name EXTENCTION!.
*/
    IplImage* img1=cvCreateImage(cvSize(640,480),IPL_DEPTH_32F,3);
    if(!cvSaveImage("outFileName.jpg",img1)) printf("Could not save: %s\n","outFileName");
}

void CvScalarExample()
{

    /*
    cvScalar() is a special case: it has three constructors. The first, called cvScalar(), takes
    one, two, three, or four arguments and assigns those arguments to the corresponding elements of val[].
    */

    CvScalar s1=cvScalar(1.0,2.0,3.0,4.0);
    cout<< s1.val[0]  <<endl;
    cout<< s1.val[1]  <<endl;
    cout<< s1.val[2]  <<endl;
    cout<< s1.val[3]  <<endl;
    ...
    The second constructor is cvRealScalar(); it takes one argument, which it assigns to val[0] while setting the other entries to 0
    */

    CvScalar s2=cvScalar(5);
    cout<< s2.val[0]  <<endl;
    cout<< s2.val[1]  <<endl;
    cout<< s2.val[2]  <<endl;
    cout<< s2.val[3]  <<endl;

    /*cvScalarAll(), which takes a single argument but sets all four elements of val[] to that 	same argument.*/
    CvScalar s3=cvScalarAll(3.2);
    cout<< s3.val[0]  <<endl;
    cout<< s3.val[1]  <<endl;
    cout<< s3.val[2]  <<endl;
    cout<< s3.val[3]  <<endl;
...
void CStyleAccessingImagePixel()
{
/*    
http://www.cs.iit.edu/~agam/cs512/lect-notes/opencv-intro/opencv-intro.html

    CvScalar, which is a set of four double-precision numbers.
    When memory is not an issue, CvScalar is often used to represent one, two, or three real numbers:
    [blue,green,red,alpha]

    Assume that you need to access the k-th channel of the pixel at the i-th-row and j-th column.
	The row index "i" is in the range [0,height-1].
	The column index "j" is in the range [0,width].
	The channel index "k" is in the range [0,nchannels-1].
*/

//Indirect access: (General, but inefficient, access to any type image)
//	For a single-channel byte image:
	int i=10,j=20;
	IplImage* img3=cvCreateImage(cvSize(640,480),IPL_DEPTH_8U,1);
	CvScalar s1;
	s1=cvGet2D(img3,i,j); // get the (i,j) pixel value
	printf("intensity=%f\n",s1.val[0]);
	s1.val[0]=111;
	cvSet2D(img3,i,j,s1); // set the (i,j) pixel value

//	For a multi-channel float (or byte) image:
	IplImage* img4=cvCreateImage(cvSize(640,480),IPL_DEPTH_32F,3);
	CvScalar s2;
	s2=cvGet2D(img4,i,j); // get the (i,j) pixel value
	printf("B=%f, G=%f, R=%f\n",s2.val[0],s2.val[1],s2.val[2]);
	s2.val[0]=111;
	s2.val[1]=111;
	s2.val[2]=111;
	cvSet2D(img4,i,j,s2); // set the (i,j) pixel value
 
}

void CStyleConvertImage()
{
/*
	cvConvertImage() is used to perform common conversions between image formats.

	The source image may be one, three, or four channels with either 8-bit or floating-point pixels.
	The destination must be 8 bits with one or three channels.
	This function can also convert color to grayscale or one-channel grayscale to three-channel grayscale (color).

	cvConvertImage(src, dst, flags=0);
	src = float or byte grayscale or color image
	dst = byte grayscale or color image
	flags = CV_CVTIMG_FLIP     (flip vertically) CV_CVTIMG_SWAP_RB  (swap the R and B channels)
*/


}

void CStyleConvertColorSpace()
{
/*
...
code
	Color conversion operation that can be specifed using CV_<src_color_space>2<dst_color_space> constants.

	CV_RGB2HSV
	CV_HSV2RGB
for more opencv ebook, Table 3-6. Conversions available by means of cvCvtColor() page 74.
...
*/        
    ...
    IplImage *imgdst_HSV;
    imgsrc_BGR=cvLoadImage(PathToImageFile.c_str(), CV_LOAD_IMAGE_UNCHANGED);
    imgdst_HSV = cvCreateImage( cvGetSize(imgsrc_BGR), imgsrc_BGR->depth, imgsrc_BGR->nChannels);
    cvCvtColor(imgsrc_BGR,imgdst_HSV,CV_BGR2HSV);
    cvSaveImage("lena_HSV.jpg",imgdst_HSV);
    cvReleaseImage(&imgdst_HSV);
    cvReleaseImage(&imgsrc_BGR);
}

void CStyleConvertScale()
{
/*    
    cvConvertScale
...
/*    
Matrices:
– CvMat // full row length in bytes
  |-- int type; //CV_<bit_depth>(S|U|F)C<number_of_channels> 
  ...
  |-- union data; //Pointer to the actual matrix data
...
*/    
    ...
    int columns=3;
    int type=CV_32F;
    CvMat* mat =cvCreateMat(rows,columns,type);//

    //or it could be defined like below
    //	CvMat mat;
    //	&mat=cvCreateMat(rows,columns,CV_32F);//

    //Accessing Data in Your Matrix
    //Indirect matrix element access:
    int i=1,j=2;
    cvmSet(mat,i,j,2.0); // Set M(i,j)
    double  t = cvmGet(mat,i,j); // Get M(i,j)

    //CV_MAT_ELEM is macro
    double element_1_2=CV_MAT_ELEM(*mat,float,1,2);
...
*/
    ...
    //When the matrix is no longer needed, it can be released by calling cvReleaseMat(CvMat**).
    cvReleaseMat(&mat);    
}
...
void CStyleMatrixAccessingElements()
{
    CvMat* c_style_mat= cvCreateMat(2,3,CV_32FC1);
    
    cvmSet(c_style_mat ,0,0,12.4);
    cvmSet(c_style_mat ,1,0,3.7);
    cvmSet(c_style_mat ,0,1,0.2);
    cvmSet(c_style_mat ,1,1,1.9);
    
    cout<<"cvmGet(c_style_mat ,0,0)=" << cvmGet(c_style_mat ,0,0)<<endl;
    cout<<"cvmGet(c_style_mat ,1,0)=" << cvmGet(c_style_mat ,1,0)<<endl;
    cout<<"cvmGet(c_style_mat ,0,1)=" << cvmGet(c_style_mat ,0,1)<<endl;
    cout<<"cvmGet(c_style_mat ,1,1)=" << cvmGet(c_style_mat ,1,1)<<endl;  
}
...
void ConvertCstyleMatrixToClassMatrix()
{
    CvMat * c_style_mat=cvCreateMat(2,2,CV_32FC1);
    cv::Mat class_matrix=c_style_mat;

    cvmSet(c_style_mat,0,0,0.0);
    cvmSet(c_style_mat,0,1,1.0);
    cvmSet(c_style_mat,1,0,2.0);
    cvmSet(c_style_mat,1,1,3.0);

    cout<<"class_matrix.at<float>(0,0)=" <<class_matrix.at<float>(0,0)<<endl;
    cout<<"class_matrix.at<float>(0,1)=" <<class_matrix.at<float>(0,1)<<endl;
    cout<<"class_matrix.at<float>(1,0)=" << class_matrix.at<float>(1,0)<<endl;
    cout<<"class_matrix.at<float>(1,1)=" <<class_matrix.at<float>(1,1)<<endl;
}

void ConvertCppMatrixToCStyleMatrix()
{
    cv::Mat class_matrix(2,2,CV_32FC1);
    class_matrix.at<float>(0,0)=0;
    class_matrix.at<float>(0,1)=1;
    class_matrix.at<float>(1,0)=2;
    class_matrix.at<float>(1,1)=3;
    
    CvMat c_style_mat =class_matrix;
}

void ConvertIPLImageToCStyleMatrix()
{
//1st approach   
    IplImage *img = cvLoadImage( "images/lena.jpg");
    CvMat *mat = cvCreateMat(img->height,img->width,img->depth );
    cvConvert( img, mat );
    
    
    
//2nd aproach    
    IplImage *img_src = cvLoadImage( "images/lena.jpg",CV_LOAD_IMAGE_COLOR);
...
// 1)matrix -CvMat*,
// 2)image - IplImage*
// 3)multi-dimensional dense array - CvMatND*
    CvMat mat_header, *dst_mat;
    dst_mat = cvGetMat( img_src , &mat_header, 0, 0);
}

void ConverCStyleMatrixToIplImage()
{
    ...
    //	1)matrix - CvMat*
    //	2)image - IplImage*.
// The function returns the image header for the input array that can be a matrix (CvMat) or image (IplImage). In the case of an image the function simply returns the input pointer. In the case of CvMat it initializes an image_header structure with the parameters of the input matrix. 
    CvMat* src_mat;
// here we should put some data in  src_mat   
    IplImage *dst_img, *image_header;
    dst_img = cvGetImage(src_mat, image_header);
    return;
}
...
void DrawingFunsction()
{
    ...
    IplImage * img=cvCreateImage(cvSize(600,400),IPL_DEPTH_8U,3);
    cvNamedWindow("WorkingwitDrawingcommands",CV_WINDOW_AUTOSIZE);
    //Draw a box:
    // draw a box with red lines of width 1 between (100,100) and (200,200)
    cvRectangle(img, cvPoint(300,100), cvPoint(200,200), cvScalar(255,0,0), 1);
    ...
    cvCircle(img, cvPoint( 350.43,100.23), 20, cvScalar(0,255,0), 1);
...
*/
    ...
    cvShowImage("WorkingwitDrawingcommands",img);
    cvWaitKey(0);
}

void CStyleResizeImage()
{

	//Resizing and displaying image
	IplImage* Image = cvLoadImage("/usr/share/doc/opencv-doc/examples/cpp/fruits.jpg");
	cvShowImage("original:",Image);
	int k=2;
	CvSize size = cvSize(Image->width/k,Image->height/k );
	int Depth=Image->depth;
	int NumberOfChannels=Image->nChannels;
	IplImage* tmpsize=cvCreateImage(size,Depth,NumberOfChannels);
...
void CppStyeAddTextToImage(cv::Mat src, std::string caption )
{
	...
	putText( dst, caption.c_str(), Point( src.cols/4, src.rows/2),CV_FONT_HERSHEY_COMPLEX, 1, Scalar(255, 255, 255) );
...
int CannyEdgeDetector_Test( char** argv )
{
    ...
    /// Convert the image to grayscale
    cvtColor( canny_edge_src, canny_edge_src_gray, CV_BGR2GRAY );
...
void CStyleSobel(IplImage *pGray)
{
...
*/
    ...
    IplImage *pSobel=cvCreateImage(cvGetSize(pGray),IPL_DEPTH_16S,1);
    int kernel_size=7;
    cvSobel( pGray,pSobel,dx_order,dy_order,kernel_size );
    cvNamedWindow("Sobel",CV_WINDOW_AUTOSIZE);
    cvShowImage("Sobel", pSobel);
    return;
}

void CStyleDisplayingCamera()
{
...
void CstylePlayingVideo( char** argv)
{
...
void cornerSubPix_Example(int argc, char ** argv)
{
    ...
    cvtColor( src_SubPix, src_gray_SubPix, CV_BGR2GRAY );
...
//            std::cout<<"found" <<std::endl;
            cv::cornerSubPix(image_gray,chess_board_corners, Size(11, 11), Size(-1, -1), TermCriteria(CV_TERMCRIT_EPS | CV_TERMCRIT_ITER, 30, 0.1));
...
{
    //CannyEdgeDetection::CannyEdgeDetector_Test(argv );
//    SobelEdgeDetector(argv);
    
    //FeatureDetection::HarrisCornerDetector_Test( argv );
    //CppStyleDisplayingCamera();
    
    //CppStyleDisplayingVideo(argv);
    
    //CstylePlayingVideo(  argv);
    
    //CppStyleDisplayingCamera();

//    CppStyleCreateMatrix();
//    MatrixTypeNumber();
//    CalculateHistogram_Test(argc,argv);
//    HOG_Test(argv);
//    HistogramEqualization_Test(argc, argv);
//    HistogramNormalization(argc,  argv);
//    draw_PCA(argc,argv);
//    remap_example(argc,argv);



//    chessboard_pose_estimation(argc, argv);

//    ProjectionMatrix_example();

//    disparity_map_using_sbm_example(argc, argv);
//    disparity_map_using_sgbm_example(argc, argv);
//    stereo_calibration(argc, argv);

//    template_matching_example(argc, argv);
//    cornerSubPix_Example(argc, argv);
    readingCameraMatrix();
}
