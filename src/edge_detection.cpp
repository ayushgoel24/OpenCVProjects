...
*/
   ...
   dx_order=1;
   dy_order=0;
   Sobel( src_gray, grad_x, depth, dx_order, dy_order, kernel_size, scale, delta, BORDER_DEFAULT );
   ...
   Sobel( src_gray, grad_y, depth, dx_order, dy_order, kernel_size, scale, delta, BORDER_DEFAULT );
   //because we can't display negative values so we have to convert the scale
